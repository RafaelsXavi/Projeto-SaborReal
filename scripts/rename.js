...
const renameMap = [
  ...
  ['apps/api/src/modules/orders/orders.courier.controller.ts', 'apps/api/src/modules/orders/orders.motoboy.controller.ts'],
  ...
  ['apps/web/src/hooks/useCourierOrders.ts', 'apps/web/src/hooks/useMotoboyOrders.ts'],
  ['apps/web/src/test/useCourierOrders.test.tsx', 'apps/web/src/test/useMotoboyOrders.test.tsx'],
  ['apps/web/src/pages/motoboy/CourierHeader.tsx', 'apps/web/src/pages/motoboy/MotoboyHeader.tsx'],
  ['apps/web/src/pages/motoboy/CourierLists.tsx', 'apps/web/src/pages/motoboy/MotoboyLists.tsx'],
];
...
function replaceInDir(dir) {
  ...
  for (const entry of entries) {
    if (entry.name === 'node_modules' || entry.name === 'dist' || entry.name === '.git' || entry.name === 'scripts' || entry.name === '.next') {
      continue;
    }
    const fullPath = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      replaceInDir(fullPath);
    } else if (entry.isFile() && (fullPath.endsWith('.ts') || fullPath.endsWith('.tsx') || fullPath.endsWith('.mjs') || fullPath.endsWith('.sql') || fullPath.endsWith('.md'))) {
